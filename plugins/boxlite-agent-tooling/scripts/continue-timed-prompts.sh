  ...
  case "$status" in
    ...
    expired)
      ...
      if [[ "$(jq -r .spec.fallback <<<"$request")" == split ]]; then
        reason='The three-minute PR size exception deadline expired. Continue now: create the parent and child issues, group them in a milestone, and split the work into coherent tested PRs of at most 400 changed lines. Do not ask again or publish the oversized diff.'
      else
