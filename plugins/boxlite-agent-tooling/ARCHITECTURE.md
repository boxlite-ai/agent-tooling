...
- Mandatory summary: `.agents/lib/concise-writing.sh` checks for an ATX TL;DR heading and summary prose outside fences, quotes, and comments. Replies must start there; the entire section has at most 39 words using the shared Unicode-aware counter. Each denial names the failed condition; an overlong section reports its word count and that it runs until the next heading. Repeated continuations cannot bypass it. Missing Stop text falls back to a bounded transcript snapshot; unreadable or truncated snapshots fail closed. Sentence simplicity remains a writing instruction.
