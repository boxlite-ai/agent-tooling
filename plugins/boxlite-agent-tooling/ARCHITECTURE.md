...
- Mandatory summary: `.agents/lib/concise-writing.sh` checks for an ATX TL;DR heading and summary prose outside fences, quotes, and comments. Replies must start there; the entire section has at most 39 words using the shared Unicode-aware counter. Denials name the failed condition and correction; length failures report the count and limit, explaining that a peer or higher-level heading ends the section. Inspection failures remain blocked. Repeated continuations cannot bypass it. Missing Stop text falls back to a bounded transcript snapshot; unreadable or truncated snapshots fail closed. Sentence simplicity remains a writing instruction.
