...
# ignored", and the dispatcher discards the result. This hook is wired synchronously, so
# it cannot block or resume; resuming belongs to resume-after-api-failure.sh, which is
# wired with asyncRewake.
#
# It records the error KIND. The payload's `error` field is a closed enum, and it is the
# ONLY place that enum is exposed: a `claude -p --output-format json` result reports
# terminal_reason "api_error" with no kind attached (the error variant's `errors` array
# is free text). Without this record an out-of-process supervisor cannot tell a revoked
# token from an overloaded server, so it either gives up on transient faults or retries
# permanent ones — resume-on-network-error.sh reads this file to avoid both.
#
# It cannot notify the human: Claude Code starts hooks detached, with no controlling
# terminal, so there is no /dev/tty to write to (2.1.278: `ps` shows none, the open fails).
#
# Everything here is best-effort by construction: a failure to record must never add a
# second failure on top of the one being reported, so every path exits 0.
set -uo pipefail
