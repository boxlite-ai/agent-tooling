...
# Exact-body fixtures captured from GitHub POST /markdown (mode=gfm), 2026-09-23.
...
case "$*" in
  'api --hostname github.com markdown -f mode=gfm -f text='*)
    ...
    jq -er --arg body "${8#text=}" 'select(.body == $body) | .html' "$DESIGN_TEST_RENDER_FIXTURES" ;;
  'api --hostname github.com repos/example/repo/issues/1')
    [[ "${DESIGN_TEST_DELETED:-0}" == 0 ]] || exit 1
    jq -nc --arg url "$DESIGN_TEST_URL" '{html_url:$url,body:"Problem: undocumented code. Approach: verify documents. Validation: gate tests."}' ;;
...
check 'missing link blocks drafts' "$draft 'A concise change.'" 'link the registered design doc'
check 'different issue cannot satisfy the link' "$draft '${DESIGN_TEST_URL}2'" 'link the registered design doc'
if [[ "${1:-}" == --reproducer ]]; then
  printf '%d passed, %d failed\n' "$pass" "$fail"; [[ "$fail" == 0 ]]; exit
fi
check 'matching draft link passes' "$draft '$DESIGN_TEST_BODY'" ''
check 'Markdown link passes' "$draft '[Design]($DESIGN_TEST_URL)'" ''
check 'Markdown link with title passes' "$draft '[Design]($DESIGN_TEST_URL \"Title\")'" ''
check 'angle-bracket Markdown destination passes' "$draft '[Design](<$DESIGN_TEST_URL> \"Title\")'" ''
for hidden_body in "[unused]: $DESIGN_TEST_URL" "[//]: # ($DESIGN_TEST_URL)" \
  ...
  $'[unused]: https://example.invalid\n"first line\n'"$DESIGN_TEST_URL"$'\nlast line"'; do
  check 'hidden references and HTML do not supply a link' "$draft '$hidden_body'" 'link the registered design doc'
done
check 'autolink passes' "$draft '<$DESIGN_TEST_URL>'" ''
# GitHub closes the outer anchor and renders this bare URL as a separate real link.
check 'renderer-created autolink passes' "$draft '<a href=\"https://example.invalid\"> $DESIGN_TEST_URL </a>'" ''
reference_body=$'[Design][doc]\n\n[doc]: '"$DESIGN_TEST_URL"
check 'used reference link passes' "$draft '$reference_body'" ''
for tag in pre code; do
  nested_body="<$tag><$tag>example</$tag><a href=\"$DESIGN_TEST_URL\">Design</a></$tag>"
  check 'nested HTML code example is not a design link' "$draft '$nested_body'" 'link the registered design doc'
done
check 'Markdown target inside nested code is not a design link' "$draft '<code><code>example</code>[Design]($DESIGN_TEST_URL)</code>'" 'link the registered design doc'
outside_body=$'<pre><pre>example</pre></pre>\n\n'"[Design]($DESIGN_TEST_URL)"
check 'link after nested code remains visible' "$draft '$outside_body'" ''
check 'code example is not a link' "$draft '\`$DESIGN_TEST_URL\`'" 'link the registered design doc'
fenced_body=$'```text\n'"$DESIGN_TEST_URL"
check 'unclosed code fence is not a link' "$draft '$fenced_body'" 'link the registered design doc'
# shellcheck disable=SC2016 # Literal Markdown backticks, never shell substitution.
for fenced_body in $'> ```\n> '"$DESIGN_TEST_URL"$'\n> ```' $'- ```\n  '"$DESIGN_TEST_URL"$'\n  ```' '``example` '"$DESIGN_TEST_URL"' `example``'; do
  check 'nested and multi-backtick examples are not links' "$draft '$fenced_body'" 'link the registered design doc'
done
check 'URL inside another URL is not a link' "$draft 'https://example.invalid/($DESIGN_TEST_URL)'" 'link the registered design doc'
check 'link label cannot disguise another target' "$draft '[$DESIGN_TEST_URL](https://example.invalid)'" 'link the registered design doc'
check 'metadata cannot supply the body link' "$draft 'Summary' --label '$DESIGN_TEST_URL'" 'link the registered design doc'
...
export DESIGN_TEST_DELETED=0 DESIGN_TEST_BODY='Missing design link.'
